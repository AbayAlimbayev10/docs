...
If you provision sandboxes in [middleware](/oss/deepagents/middleware), there is no `on_error` hook for cleanup when exceptions occur. As a workaround:
...
1. **Set a TTL** for the sandbox—most providers support this. Use it even on the happy path; it's especially important when cleanup might not run.
2. **Use `wrap_model_call` and `wrap_tool_call` hooks** to catch unexpected exceptions and clean up resources. Note that this won't help for node-style hooks (`before_agent`, `after_agent`, `before_model`, `after_model`) if the exception arises from other middleware.
