...
1. **Filters**: Located at the top left of the **Tracing** project page. This is where you construct and manage filter criteria.
    ...
    - [Time range filter](#filtering-by-time-range).
...
<Info>
By default, the **Traces** filter is applied, which displays only top-level root runs. You can switch to **Runs** to show all runs, including intermediate spans, in the project.
</Info>

### Filter by time range

You can also filter runs within a specific time range. By default this is set to the last 7 days, but you can change the time increment and select a calendar date to customize.

If you want to save a filter within a specific time range, you can select your filter attributes and the time range before selecting **Save** on the view. For example, you may want to have a view of error messages to review each morning.

<img
    className="block dark:hidden"
    src="/langsmith/images/time-range-filter-light.png"
    alt="LangSmith time range filter showing date and time selection options"
/>

<img
    className="hidden dark:block"
    src="/langsmith/images/time-range-filter-dark.png"
    alt="LangSmith time range filter showing date and time selection options"
/>
...
To filter for intermediate runs (spans), change the default from **Traces** to **Runs**. For example, you would do this if you wanted to filter by **run name** for sub runs or filter by **run type**.
...
A common concept is to filter for intermediate runs which are part of a trace whose root run has some attribute. An example is filtering for intermediate runs of a particular type whose root run has positive (or negative) feedback associated with it. To do this:

1. Set up a [filter for intermediate runs](#filter-for-intermediate-runs-spans).
